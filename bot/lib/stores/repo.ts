...
// Error handling wrapper for any repository
export class ErrorHandlingRepository<Input, Ctx, Output>
  implements Repository<Input, Ctx, Output>
{
  constructor(
    private readonly underlyingRepository: Repository<Input, Ctx, Output>
  ...
  public async read(input: Input, context?: Ctx): Promise<Output | null> {
    try {
      return await this.underlyingRepository.read(input, context);
    } catch (error) {
...
// Helper function to wrap any repository with error handling
export function withErrorHandling<Input, Ctx, Output>(
  repository: Repository<Input, Ctx, Output>
): Repository<Input, Ctx, Output> {
  return new ErrorHandlingRepository(repository);
}
